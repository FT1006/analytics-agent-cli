...
def find_datasources(working_directory: str, directory_path: str = ".") -> str:
    """Discover available data files and present them as load options."""
    try:
        # Get the current working directory or specified path
        # Use working_directory as base if directory_path is relative
        if not os.path.isabs(directory_path):
            current_dir = Path(working_directory) / directory_path
        else:
...
schema_find_datasources = types.FunctionDeclaration(
    name="find_datasources",
    description="Discover available data files and present them as load options",
    parameters=types.Schema(
        type=types.Type.OBJECT,
        properties={
            "directory_path": types.Schema(
                type=types.Type.STRING,
                description="Directory path to search for data files (defaults to current directory)",
            ),
        },
        required=[],
    ),
